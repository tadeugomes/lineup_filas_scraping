...
;
!function(a){Drupal.behaviors.RemoveRedesSociaisFooter={attach:function(o,t){a("footer#main-footer .footer-social",o).on("load",function(){a(this).css({display:"none"}),a("footer#main-footer .footer-logo-celepar").css({"margin-top":"0"})}).once("footerSocial").trigger("load")}}}(jQuery);;
/**
...
;
(function($) {
  Drupal.behaviors.celeparAgendaExecutiva = {
    attach: function(context, settings) {

      $("#selectUsuario").on('change', function() {
        var valor = $(this).val();
        if (valor !== "") {
          $('<form action="/webservices/agenda-executiva/" method="POST"/>').append($('<input type="input" name="dados">').val(valor)).appendTo($(document.body)).submit();
        }
      });

      // Envio dos dados por post para atualizar o mes do calendario ( botões "<<"  "mes_corrente"  ">>" )
      $(".NavMonth", context).on('click', function() {
      ...
      // Cliquei em uma data no bloco, redirecione para a página da agenda, na data específica
      $(".btn-days-bloco", context).on('click', function() {
        $(location).prop('href', '/webservices/agenda-executiva#' + $(this).attr('data-value'));
      });
    }
  };
})(jQuery);
